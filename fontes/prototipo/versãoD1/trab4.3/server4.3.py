            ...
            with context.wrap_socket(conn, server_side=True) as secure_conn:
                ...
                try:
                    ...
                    if client_otp_hash in valid_otp_hashes:
                        print(f"Sending response: OTP Valid")
                        ...
                        print(f"Sending response: OTP Invalid")
                        secure_conn.send(b"OTP Invalid")
                        print("OTP validation failed")
                        continue

                    # Enviar o contador atualizado
                    print(f"Sending synchronized counter: {server_counter}")
                    secure_conn.send(str(server_counter).encode()) 
                    ...
                    obfuscated_cred = obfuscate_credential(password, seed, server_counter)
                    
                    # Salva a comparação em credential.txt
                    save_credential_comparison(password, obfuscated_cred)
