...
#    return base64.b64encode(''.join(chr(ord(c) ^ key[i % len(key)]) for i, c in enumerate(credential)).encode()).decode()
    ...
    key = hashlib.pbkdf2_hmac('sha256', otp.encode(), nonce, 100000)  # Deriva uma chave usando o OTP e o nonce
   
    username, password = credential  # Desempacota a tupla
    key = [ord(c) for c in otp]  # Converte o OTP em uma lista de valores ordinais

    # Ofusca o nome de usuário e a senha individualmente
    obfuscated_username = ''.join(chr(ord(c) ^ key[i % len(key)]) for i, c in enumerate(username))
    obfuscated_password = ''.join(chr(ord(c) ^ key[i % len(key)]) for i, c in enumerate(password))

    # Junta o nome de usuário e senha ofuscados
    combined_obfuscated = f"{obfuscated_username}:{obfuscated_password}"

    # Codifica o resultado em Base64 para garantir uma transmissão segura
    return base64.b64encode(combined_obfuscated.encode()).decode()
        ...
        while True:
            #with context.wrap_socket(sock, server_side=True) as secure_sock:
                ...
                with conn:
                    ...
                    # Obter-Gerar credencial no Vault
                    ...
                    # Exibir credencial obtida do Vault
                    print(f"Credencial obtida do Vault: Usuário: {credential[0]}, Senha: {credential[1]}")

                    # Ofuscar credencial
                    obfuscated_cred = obfuscate_credential(credential, server_otp)

                    # Exibir credencial ofuscada
                    print(f"Credencial ofuscada: {obfuscated_cred}")
